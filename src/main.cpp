/**
 * copied from the example as described below and chaned things. Sorry, don't remember what :-(
 *
...
void start(){
  ...
  {
    Serial.println("write block0 to card");
    writeBlock0();
  }
...
void writeBlock0(){ 
  ...
  for (byte i = 0; i < 6; i++) {
    ...
    Serial.print(F("Writing data into block 0 \n ")); 


  byte block0_buffer[18];
  // Set new UID
  byte newUid[] = NEW_UID;
  uint8_t uidSize = mfrc522.uid.size;
  ...
  block0_buffer[uidSize] = bcc;
    ...
    dump_byte_array(newUid, 16); 


    status = (MFRC522::StatusCode) mfrc522.MIFARE_Write(block, block0_buffer, 16);
    if (status != MFRC522::STATUS_OK) {
